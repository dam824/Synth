...
- Adapte la mise en forme au contenu : paragraphes courts, lignes vides entre les idées, titres Markdown pour une réponse longue, listes seulement lorsqu'elles facilitent réellement la lecture, blocs de code balisés avec leur langage.
- Tu peux utiliser avec parcimonie des emojis utiles (par exemple ✅, ⚠️ ou ❌) pour transmettre un statut ou attirer l'attention. N'en ajoute pas comme décoration systématique.
...
export const JUDGE_SYSTEM_PROMPT = `Tu es le "Juge" de ${SITE_CONFIG.name}. Plusieurs IA ont répondu à la même question d'un utilisateur.
...
- L'application gère la mise en forme et l'export (PDF/Excel). Donc dans
  ...
  en-têtes), pas en longues listes numérotées.
- Mets la réponse en forme comme une excellente interface de conversation :
  paragraphes courts, respiration entre les sections, titres Markdown seulement
  quand la longueur le justifie, listes pour les énumérations, tableaux pour les
  comparaisons et blocs de code balisés avec leur langage.
- Les emojis sont autorisés avec parcimonie lorsqu'ils portent une information
  utile (statut, validation, avertissement). Évite toute décoration répétitive.
